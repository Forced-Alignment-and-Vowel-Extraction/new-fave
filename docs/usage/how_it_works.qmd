...
For these 4 distributions, the Mahalanobis distance is calculated for each candidate formant track for each vowel token. These Mahalanobis distances are converted to log-probabilities and summed, resulting in an overall log-probability for each candidate track. The candidate track with the largest log-probability is selected as the new winning track.

This optimization is repeated for a maximum of 10 iterations, or until the difference between two automation steps or until the difference in formant tracks is sufficiently small.
